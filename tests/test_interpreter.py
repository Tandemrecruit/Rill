...
def test_repeat_for_range_empty_range():
    """
    Given a for-range with start > end and no step for ascending or start == end, the loop body should not execute.
    """
    # Empty ascending range (start > end, no step provided would error, so using explicit step 1)
    src = """set count to 0
repeat for i from 5 to 3 step 1
...
"""
    ...
    # Empty range with start == end (exclusive until)
    src2 = """set count to 0
repeat for i from 5 until 5
    change count to count + 1
end
show count
"""
    ...
    # Single iteration with start == end (inclusive to)
    src3 = """set count to 0
...
show i
"""
    with pytest.raises(RillRuntimeError, match="not defined"):
        run_src(src)
...
"""
    out2 = run_src(src2)
    assert out2 == ["3"]  # 2+1+0+(-1)+(-2)+(-3) = -3... wait let me recalculate
    # Actually: 2+1+0-1-2-3 = -3, but let me trace it properly
    # i: 2, 1, 0, -1, -2, -3 = sum is -3
    out2 = run_src(src2)
    assert out2 == ["-3"]
...
set multiplier to 2
repeat for i from 1 to 3
    change result to result + [i * multiplier]
end
show result[0]
show result[1]
show result[2]
